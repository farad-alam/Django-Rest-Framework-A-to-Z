...
class student_api(View):
    ...
    def get(self,request,*args, **kwargs):
        ...
        if id is not None:
            ...
            serialize = StudentModelSerializer(student_data)
        ...
        serialize = StudentModelSerializer(all_students,many=True)
        python_to_json_data = JSONRenderer().render(serialize.data)
        return HttpResponse(python_to_json_data, content_type='application/json')
    ...
    def post(self,request,*args, **kwargs):
        ...
        stream = io.BytesIO(json_data)
        python_data = JSONParser().parse(stream)
        serialize = StudentModelSerializer(data=python_data)
    ...
    def put(self,request,*args, **kwargs):
        ...
        if id is not None:
            student_data = Students.objects.get(id=id)
            serialize = StudentModelSerializer( student_data, data=python_data, partial=True)
            if serialize.is_valid():
                serialize.save()
